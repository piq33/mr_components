...
///    system.
class SafeAreaObservable extends StatelessWidget {
  /// Creates a widget that avoids operating system interfaces.
  ///
  /// The [left], [top], [right], [bottom], and [minimum] arguments must not be
  /// null.
  const SafeAreaObservable({
    super.key,
    this.left = true,
    this.top = true,
    this.right = true,
  ...
  /// Specifies whether the [SafeAreaObservable] should maintain the bottom
  ...
  Widget build(BuildContext context) {
    ...
    final observeLeft = math.max(padding.left, minimum.left);
    final observeTop = math.max(padding.top, minimum.top);
    final observeRight = math.max(padding.right, minimum.right);
    final observeBottom = math.max(padding.bottom, minimum.bottom);
    observable?.call(observeLeft, observeTop, observeRight, observeBottom);

    final actualLeft = math.max(left ? padding.left : 0.0, minimum.left);
    final actualTop = math.max(top ? padding.top : 0.0, minimum.top);
    final actualRight = math.max(right ? padding.right : 0.0, minimum.right);
    final actualBottom =
...
/// See also:
///
///  * [SafeAreaObservable], for insetting widgets to avoid operating system intrusions.
